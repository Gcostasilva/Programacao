...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($acao === 'salvar') {
            ...
            header('Location: index.php?page=vendedores&status=salvo');
            exit;
        }
        ...
        if ($acao === 'excluir') {
            ...
            if ($referencias > 0) {
                throw new RuntimeException("Não é possível excluir: o vendedor possui {$referencias} registro(s) de programação vinculado(s). Desative-o em vez de excluir.");
            }

            $model->excluir($id);
            header('Location: index.php?page=vendedores&status=excluido');
            exit;
        }
    } catch (Throwable $e) {
