        ...
        function filtrarTabela() {
            ...
            function atualizarIndicadores(dados) {
                ...
                for (const id in mapa) {
                    document.getElementById(id).textContent = mapa[id] ?? '--';
                }
            }
            const recursof = document.getElementById('recurso').value;
            const dataf = document.getElementById('data').value;

            if (recursof && dataf) {

                fetch(`index.php?page=prog_diaria_dados&equipamento=${recursof}&data=${dataf}`)
                    .then(resp => {
                        if (!resp.ok) {
...
<?php if ($pagina === 'prog_semanal'): ?>
                ...
                fetch(`index.php?page=prog_semanal_buscarCodigo&codigo=${encodeURIComponent(codigo)}`)
                    ...
                    .then(data => {
                        ...
                        if (data.descricao !== undefined) {
                            ...
                            espessuraInput.value = data.espessura;
                        }
            ...
            modalEditarSemanal.addEventListener('show.bs.modal', function (event) {
                ...
                fetch("index.php?page=prog_semanal_buscar&id=" + id)
                    ...
                    .then(function (registro) {
                        ...
                        document.getElementById('edit_descricao_COMP').value = registro.desc_complementar;
                        document.getElementById('edit_sem_quantidade').value = registro.qtd;
                        document.getElementById('edit_sem_peso').value = registro.peso;
                        document.getElementById('edit_sem_descricao').value = registro.descricao + registro.desc_complementar;
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const espessuraModal = document.getElementById('espessura_chapa');

            if (!modalCalculoChapa || !espessuraSemanal || !espessuraModal) {
                return;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            btnAplicarCalculoPerfil.addEventListener('click', function (e) {
                ...
                const quantidadePrincipal_perfil = document.getElementById('quantidade_perfil');
                const pesoPrincipal_perfil = document.getElementById('peso_perfil');
                const texto_complementar = document.getElementById('texto_complementar_perfil');


                // ==================================================
                // VALIDAÇÃO
                // ==================================================
                if (!quantidadePrincipal_perfil || !pesoPrincipal_perfil || !texto_complementar) {
                    console.error('Campos do cálculo de perfil não encontrados.');
                    return;
                }


                if (quantidadePrincipal_perfil.value === '' || pesoPrincipal_perfil.value === '' ||
                    isNaN(quantidadePrincipal_perfil.value) || isNaN(pesoPrincipal_perfil.value)
                ) {
                    alert(
                        'Preencha corretamente os campos de quantidade e peso.'
                    );
                ...
                // ==================================================
                // ENVIA O RESULTADO PARA O CONTEXTO ATUAL
                // ==================================================

                document.getElementById(contextoCalculo.quantidade).value = quantidadePrincipal_perfil.value;
                document.getElementById(contextoCalculo.peso).value = pesoPrincipal_perfil.value;

                // ==================================================
                // DESCRIÇÃO
                // ==================================================

                document.getElementById(contextoCalculo.descricao).value += texto_complementar.value;
                document.getElementById(contextoCalculo.descricao_comp).value = ` ${texto_complementar.value}`;

                // ==================================================
                // FECHA O MODAL
                // ==================================================

                const modalElement_perfil = document.getElementById('modalCalculo_perfil');
                const modal_perfil = bootstrap.Modal.getInstance(modalElement_perfil);
                ...
                // ==================================================
                // DEVOLVE O FOCO PARA O CAMPO ORIGINAL
                // ==================================================

                const campoFoco = document.getElementById(contextoCalculo.foco);
