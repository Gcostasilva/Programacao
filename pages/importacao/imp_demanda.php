...
function limparBanco($pdo) {
    ...
    $pdo->exec("DELETE FROM demanda");
    $pdo->exec("ALTER TABLE demanda AUTO_INCREMENT = 1");
...
function importarCsvParaBanco($caminhoArquivo, $pdo)
{
    ...
    $stmt = $pdo->prepare("INSERT INTO demanda (grupo, codigo, armazem, estoque, pendencia) VALUES (:grupo, :codigo, :armazem, :estoque, :pendencia)");
    ...
    while (($dados = fgetcsv($handle, 1000, ';')) !== false) {
        ...
        $stmt->execute([
            ':grupo' => trim($dados[0]),
            ':codigo' => trim($dados[1]),
            ':armazem' => trim($dados[2]),
            ':estoque' => trim($dados[3]),
            ':pendencia' => trim($dados[4])
        ]);
        $linhasImportadas++;
    }
    ;
