...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function formatarTipo(tipo) {
            ...
            if (valor === 'P') return 'Padrão';
            if (valor === 'E') return 'Encomenda';
            if (valor === 'T') return 'Telha';
            return valor || '-';
        }
        ...
        async function consultarPedido() {
            ...
            try {
                const resposta = await fetch('index.php?page=pedidos_buscar&pedido=' + encodeURIComponent(pedido), {
                    cache: 'no-store'
                });
        ...
        tabela.addEventListener('click', async function(event) {
            ...
            try {
                const resposta = await fetch('index.php?page=pedidos_comentario&acao=listar&pedido_id=' + encodeURIComponent(pedidoIdComentario), {
                    cache: 'no-store'
                });
                if (!resposta.ok) throw new Error('HTTP ' + resposta.status);
        ...
        btnSalvarComentario.addEventListener('click', async function() {
            ...
            try {
                ...
                const resposta = await fetch('index.php?page=pedidos_comentario', {
                    method: 'POST',
                    body: dados
                });
