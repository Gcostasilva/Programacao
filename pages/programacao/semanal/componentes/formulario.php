        ...
        <div class="card-body">
            <form action="index.php?page=prog_semanal_novo" method="POST" id="formProgramacaoSemanal" autocomplete="off">
                <div class="row align-items-end">
                    ...
                    <div class="col-md-3"><label class="form-label">Código</label><div class="input-group"><input type="text" class="form-control" name="codigo_s" id="codigo" autocomplete="off" required><button class="btn btn-primary" type="button" id="btn_buscaCodigo" title="Selecionar código de produto"><i class="bi bi-search"></i></button><button class="btn btn-primary ms-1" type="button" id="btn_buscaDemanda" title="Selecionar demanda"><i class="bi bi-boxes"></i></button></div></div>
...
<div class="modal fade" id="modalCodigosProgramacao" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title"><i class="bi bi-search me-2"></i>Selecionar produto</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const descricao = document.getElementById('descricao_sem');
    const peso = document.getElementById('peso');
    const btnCodigo = document.getElementById('btn_buscaCodigo');
    const modalEl = document.getElementById('modalCodigosProgramacao');
    const filtro = document.getElementById('filtroCodigosProgramacao');
    const chave = 'programacaoSemanal_filtros';

    if (semana && recurso && data && form) {
        try { const salvo = JSON.parse(localStorage.getItem(chave) || '{}'); if (salvo.semana) semana.value = salvo.semana; if (salvo.recurso) recurso.value = salvo.recurso; if (salvo.data) data.value = salvo.data; } catch (e) {}
        const persistir = () => localStorage.setItem(chave, JSON.stringify({ semana: semana.value, recurso: recurso.value, data: data.value }));
        semana.addEventListener('change', persistir); recurso.addEventListener('change', persistir); data.addEventListener('change', persistir);
    }

    if (modalEl && btnCodigo) {
        const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
        btnCodigo.addEventListener('click', () => modal.show());
    }

    document.querySelectorAll('.btn-selecionar-codigo').forEach(function (botao) {
        botao.addEventListener('click', function () {
            if (codigo) codigo.value = this.dataset.codigo || '';
            if (descricao) descricao.value = this.dataset.descricao || '';
            if (peso && this.dataset.peso) peso.value = this.dataset.peso;
    ...
    filtro?.addEventListener('input', function () {
        ...
        document.querySelectorAll('#tabelaCodigosProgramacao tbody tr').forEach(linha => { linha.hidden = !!termo && !linha.dataset.busca.includes(termo); });
    });
});
</script>
