...
function periodoDaSemana(string $semana): array
{
    ...
    $n = (int)$m[2];
    if ($n < 1 || $n > 53) throw new InvalidArgumentException('Número de semana inválido.');
    $segunda = new DateTime();
    $segunda->setISODate((int)$m[1], $n, 1);
    $sexta = clone $segunda;
    $sexta->modify('+4 days');
    return [$segunda->format('Y-m-d'), $sexta->format('Y-m-d')];
}
function rsData(string $data): string
{
    return (new DateTime($data))->format('d/m/Y');
}
function rsNumero(float $valor): string
{
    return number_format($valor, 0, ',', '.');
}
function rsDia(string $data): string
{
    $map = ['Sunday' => 'Domingo', 'Monday' => 'Segunda-feira', 'Tuesday' => 'Terça-feira', 'Wednesday' => 'Quarta-feira', 'Thursday' => 'Quinta-feira', 'Friday' => 'Sexta-feira', 'Saturday' => 'Sábado'];
    return $map[(new DateTime($data))->format('l')] ?? '';
}
try {
...
<div class="container-fluid py-3 rp04-page">
    <div class="d-flex justify-content-between align-items-center mb-3 rp04-toolbar">
        <div>
            <h2 class="mb-1">Relatório de Programação — Semanal</h2>
            <div class="text-muted small">Modelo RP 04 — Programação de produção</div>
        </div>
    ...
    <form method="get" class="card shadow-sm mb-3 rp04-toolbar"><input type="hidden" name="page" value="relatorio_programacao">
        <div class="card-body">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-5"><label class="form-label">Equipamento</label><select name="recurso_id" class="form-select">
                        <option value="">Todos os equipamentos</option><?php foreach ($recursos as $r): ?><option value="<?= (int)$r['id'] ?>" <?= $recursoId === (int)$r['id'] ? 'selected' : '' ?>><?= htmlspecialchars($r['descricao']) ?></option><?php endforeach; ?>
                    </select></div>
    ...
    <?php if ($erro): ?>
        <div class="alert alert-danger rp04-toolbar"><?= htmlspecialchars($erro) ?></div>
    <?php elseif ($relatorio && empty($relatorio['equipamentos'])): ?>
        <div class="alert alert-info rp04-toolbar">Não há programação para a semana e equipamento selecionados.</div>
        ...
        foreach ($relatorio['equipamentos'] as $equipamento): $totalEquipamentoQuantidade = 0;
            ...
            <article class="rp04-document">
                <header class="rp04-header">
                    <div class="rp04-top">
                        <div class="rp04-logo">
                            <div class="rp04-logo-mark">P</div>
                            <div><strong>PERFINASA</strong><small>PERFILADOS DE AÇO</small></div>
                        </div>
                        <div class="rp04-system"><strong>Sistema de Gestão da Qualidade</strong><span>REGISTRO DE PRODUÇÃO</span></div>
                        <div class="rp04-code"><strong>Identificação: RP 04</strong><span>Revisão: 04</span></div>
                    </div>
                    <div class="rp04-title">PROGRAMAÇÃO DE PRODUÇÃO</div>
                    <div class="rp04-date"><span><strong>Data Prevista Inicial:</strong>
                            <?= rsData($relatorio['periodo']['inicio']) ?></span>
                        <span><strong>Data Prevista Final:</strong> <?= rsData($relatorio['periodo']['fim']) ?></span>
                ...
                <section class="rp04-content">
                    <table class="rp04-table">
                        ...
                        <tbody>
                            <?php foreach ($equipamento['dias'] as $dia): ?><tr class="rp04-day-row">
                                    <td colspan="5"><strong><?= htmlspecialchars(rsDia($dia['data'])) ?> - <?= rsData($dia['data']) ?></strong><span>Total do dia: <?= rsNumero((float)$dia['totais']['quantidade']) ?> peças / <?= rsNumero((float)$dia['totais']['peso_estimado']) ?> kg</span></td>
                                </tr><?php foreach ($dia['itens'] as $item): ?><tr class="rp04-item-row">
                                        <td><?= htmlspecialchars((string)$item['produto_id']) ?></td>
                                        <td><?= htmlspecialchars($item['descricao']) ?><?php if (!empty($item['observacao'])): ?><small>Obs.: <?= htmlspecialchars($item['observacao']) ?></small><?php endif; ?></td>
                                        <td><?= htmlspecialchars((string)($item['demanda'] ?? '')) ?></td>
                                        <td class="num"><?= rsNumero((float)$item['quantidade']) ?></td>
                                        <td class="num"><?= rsNumero((float)$item['peso_estimado']) ?></td>
                                    </tr><?php endforeach; ?><?php endforeach; ?>
                        </tbody>
                    </table>
                </section>
                <footer class="rp04-footer">
                    <div class="rp04-total-equipamento"><span>TOTAL PROGRAMADO — <?= htmlspecialchars($equipamento['nome']) ?></span><span><?= rsNumero($totalEquipamentoQuantidade) ?> PEÇAS</span><span><?= rsNumero($totalEquipamentoPeso) ?> KG</span></div>
            ...
            <?php endforeach; ?><?php if ($recursoId === null): ?><div class="rp04-total-semana"><span>TOTAL DA SEMANA</span><span><?= rsNumero($totalSemanaQuantidade) ?> PEÇAS</span><span><?= rsNumero($totalSemanaPeso) ?> KG</span></div><?php endif; ?><?php endif; ?>
</div>
<style>
    .rp04-page {
